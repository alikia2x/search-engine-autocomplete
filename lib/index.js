...
async function completeGoogle(query, lang='en-US') {
    ...
    try {
        ...
        let verbatimRelevance = result[4]["google:verbatimrelevance"];
        if (verbatimRelevance === undefined) {
            verbatimRelevance = result[4]["google:suggestrelevance"].reduce((max, item) => Math.max(max, item), -Infinity);
        }
